...
class _PageCodeViewState extends State<PageCodeView> {
  ...
  late String? _validatorMsg = barcodeValidator(
  ...
  HistoryErrorLevel _initErrorLevel() {
    ...
    return (historyErrorLevel == HistoryErrorLevel.none ||
            historyErrorLevel == null)
  ...
  Future<void> _pressExport(String fileSuffix) async {
    try {
      ...
      switch (fileSuffix) {
        ...
        default:
          throw 'Unsupported file format: $fileSuffix';
      }
        ...
        actions: _historyFormat != null && _validatorMsg == null
            ? [
                MyMenuButton(
                  ...
                  items: [
                    for (final String suffix in const [
                      StaticString.pngSuffix,
                      StaticString.jpgSuffix,
                      StaticString.svgSuffix,
                    ])
                      MyMenuItem(
                        text: suffix.toUpperCase(),
                        onTap: () => _pressExport(suffix),
                      ),
                  ],
                ),
                IconButton(
                      ...
                      child: LayoutBuilder(
                        builder: (context, constraints) {
                          final double width = isPortrait
                              ? min(
                                  constraints.biggest.shortestSide * 0.8,
                                  longestSide * 0.5,
                                )
                              : constraints.biggest.shortestSide;
                          Widget? svgWidget;
                          try {
                            if (_validatorMsg == null) {
                              svgWidget = SvgPicture.string(
                                _getBarcodeSvg(width),
                              );
                            }
                          } catch (e) {
              ...
              Expanded(
                ...
                child: ListView(
                  children: [
                    ...
                    if (_historyFormat == HistoryFormat.qrCode)
                      Text(
  ...
  String _getBarcodeSvg(double width) => _getBarcode().toSvg(
    _historyItem.contents,
    width: width,
    height: _getHeight(width),
  );

  img.Image _getBarcodeImage(double width) {
    final img.Image image = img.Image(
      width: width.round(),
      height: _getHeight(width).round(),
    );
  ...
  Barcode _getBarcode() =>
      _historyFormat == HistoryFormat.qrCode || _historyFormat == null
      ? Barcode.qrCode(
          errorCorrectLevel:
              _historyErrorLevel.barcodeQRCorrectionLevel ??
