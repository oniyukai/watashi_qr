...
class _PageItemViewState extends State<PageItemView> {
      ...
      ElevatedButton(
        ...
        onPressed: () {
          ...
          _historyItem.contents =
              _formKey.currentState!.value['modifyContents'];
          _historyItem.type = HistoryType.fromDistinguish(
            _historyItem.getFormat,
            _historyItem.contents,
          ).name;
            ...
            children: [
                            ...
                            children: [
                              SelectableText(
                                DictKey.analysisLabelFormat.s +
                                    HistoryFormat.localeStrFromName(
                                      _historyItem.format,
                                    ),
                              ),
                              SelectableText(
                                Utils.formatUnixTimes(_historyItem.unixTime),
                              ),
                            ],
                          ),
                          SelectableText(
                            DictKey.analysisLabelOrigin.s +
                                HistoryOrigin.localeStrFromName(
                                  _historyItem.origin,
                                ),
                          ),
                          if (_historyItem.getErrorLevel !=
                              HistoryErrorLevel.none)
                            SelectableText(
                              DictKey.analysisLabelErrorLevel.s +
                                  HistoryErrorLevel.localeStrFromName(
                                    _historyItem.errorLevel,
                                  ),
                            ),
                          if (_historyItem.notes.isNotEmpty)
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
              ...
              Card(
                child: ListTile(
                  ...
                  onTap: () =>
                      context.routeOf<PageCodeView>().toPass(_historyItem),
                  trailing: Row(
              ...
              Builder(
                builder: (context) {
                  ...
                  final List<PressButtonGrid> actionGrids =
                      _getActionGridList();
                  final int rowCount = (actionGrids.length / crossAxisCount)
                      .ceil();
                  return Column(
                      ...
                      return IntrinsicHeight(
                        child: Row(
                          ...
                          children: List.generate(crossAxisCount, (
                            ...
                            final int index =
                                rowIndex * crossAxisCount + columnIndex;
  ...
  List<PressButtonGrid> _getActionGridList() => [
    if (_historyType != HistoryType.website)
      PressButtonGrid(
        iconData: Icons.search,
        description: DictKey.actionWebSearch.s,
        onTap: () {
          final SearchEngine searchEngine = context.readPrefs.get(
    ...
    if (context.readPrefs
        .get<List<CustomSearchUrl>>(.customSearchUrls)
        .isNotEmpty)
      PressButtonGrid(
        iconData: Icons.search,
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () async {
              if (_formKey.currentState?.saveAndValidate() != true) return;
              _historyItem.notes =
                  _formKey.currentState!.value['modifyNotes'] ?? '';
              Navigator.pop(context);
              _syncToDatabase();
            },
          ),
    ...
    // ),
    if (_historyType == HistoryType.contact)
      PressButtonGrid(
    ...
    if (_historyType == HistoryType.mail)
      PressButtonGrid(
    ...
    if (_historyType == HistoryType.phone || _historyType == HistoryType.sms)
      PressButtonGrid(
    ...
    if (_historyType == HistoryType.location)
      PressButtonGrid(
    ...
    // if (_historyType == HistoryType.event) PressButtonGrid(
